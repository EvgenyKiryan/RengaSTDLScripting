...
Все идентификаторы системных перечислений хранятся в таблице ``renga``. Получить доступ к ним в скриптовой части можно следующим способом:

.. code-block:: lua
    :caption: Пример 1. Присвоение локальной переменной ``connector_type`` значения системного перечисления
    :linenos:
    
    local connector_type = renga.connector_type.weld
